//ezek a modulok elerese - betoltese - a require() metodussal
...
function reqListener(req, resp) {
    ...
    if (url === '/') {
        ...
        resp.write('</html>');
        //hogy ne fusson tovabb a valasz ezert returnal kell lezarni - mert ez egy callback funkcio
        return resp.end();
    }
    ...
    if (url === '/message' && method === 'POST') {
        ...
        req.on('end', () => {
            ...
            beirando = parosito.split('=')[1];
            fsys.writeFile('message.txt', beirando, err => {
                resp.statuCode = 302;
    ...
    resp.write('<body><h1>ZOLO</h1><p>megjott a valasz</p>');
    resp.write('<p> csak nemlatom mi az.... viszont beirtam a message.txt fileba</p>');
